...
<div id="wrapper">
    ...
    <div class="container">
        <div class="row">
            <div class="col-xs-12">
                <form id="frmQuery" class="form-inline hot-search" action="${pageContext.request.contextPath}/front/product/main" method="post">
                    <input type="hidden" name="pageNum" id="pageNum"/>
                    <div class="form-group">
                        <label class="control-label">商品：</label>
                        <input type="text" class="form-control" placeholder="商品名称" name="name" value="${productParams.name}">
                    </div>
                    &nbsp;
                    <div class="form-group">
                        <label class="control-label">价格：</label>
                        <input type="text" class="form-control" placeholder="最低价格" name="minPrice" value="${productParams.minPrice}"> --
                        <input type="text" class="form-control" placeholder="最高价格" name="maxPrice" value="${productParams.maxPrice}">
                    </div>
                    &nbsp;
                    <div class="form-group">
                        ...
                        <select class="form-control input-sm" name="productTypeId">
                            ...
                            <c:forEach items="${productTypes}" var="productType">
                              <option value="${productType.id}" <c:if test="${productType.id==productParams.productTypeId}">selected</c:if>>${productType.name}</option>
                            </c:forEach>
                        </select>
                    </div>
                    &nbsp;
